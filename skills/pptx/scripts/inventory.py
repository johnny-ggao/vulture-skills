...
class ShapeData:
    ...
    def get_font_path(font_name: str) -> Optional[str]:
        ...
        if not font_name:
            return None
        if font_name in ShapeData._FONT_PATH_CACHE:
            return ShapeData._FONT_PATH_CACHE[font_name]
        ...
        for font_dir in font_dirs:
            ...
            for variant in font_variations:
                for ext in extensions:
                    ...
                    if font_path.exists():
                        ShapeData._FONT_PATH_CACHE[font_name] = str(font_path)
                        return str(font_path)

            # Then try fuzzy matching - find files containing the font name.
            # Recurse one directory tree because macOS keeps many useful fonts
            # in /System/Library/Fonts/Supplemental.
            try:
                for file_path in font_dir_path.rglob("*"):
                    if file_path.is_file():
                        ...
                        if font_name_lower in file_name_lower and any(
                            ...
                            ShapeData._FONT_PATH_CACHE[font_name] = str(file_path)
                            return str(file_path)
            except (OSError, PermissionError):
                continue

        ShapeData._FONT_PATH_CACHE[font_name] = None
        return None
    ...
    def _wrap_text_line(
        self, line: str, max_width_px: int, draw, font, width_factor: float
    ) -> List[str]:
        ...
        for token in tokens:
            ...
            if measure(test_line) <= max_width_px:
    ...
    def _estimate_frame_overflow(self) -> None:
        ...
        for para_idx, paragraph in enumerate(text_frame.paragraphs):
            ...
            for line in text.split("\n"):
                wrapped = self._wrap_text_line(
                    line, para_width_px, draw, font, width_factor
                )
        ...
        thresh_right = 0.06 if autofit_mode == "normAutofit" else 0.03
        thresh_bottom = 0.16 if autofit_mode == "normAutofit" else 0.08

        overflow_right = round(bbox_right - frame_right, 2)
        if overflow_right > thresh_right:
