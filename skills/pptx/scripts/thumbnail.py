...
def _soffice_missing_message(pptx_path, exists: bool = False) -> str:
    """Build a clear fallback message when soffice is missing or fails.

    `exists=False` means the executable is not on PATH; `exists=True` means
    soffice ran but the PDF was not produced.
    """
    header = (
        "soffice (LibreOffice) is required for PowerPoint -> PDF -> JPG thumbnails."
        if not exists
        else "soffice produced no PDF for this file."
    )
    return (
        f"{header}\n\n"
        "Fallbacks (no LibreOffice needed):\n"
        "  1. For ANY .pptx, run the generated deck through the text layout\n"
        "     checker. In the html2pptx workflow this is the hard delivery gate\n"
        "     when real PPTX thumbnails are unavailable:\n"
        f"         python scripts/inventory_textgrid.py {pptx_path} \\\n"
        f"             --issues-only --fail-on-issues\n"
        "     This loses pixel fidelity, but it checks the output .pptx and\n"
        "     surfaces estimated text overflow / overlap issues.\n\n"
        "  2. If you are in the html2pptx workflow and still have the source HTML\n"
        "     slides, screenshot them with Playwright (already an html2pptx\n"
        "     dependency) as an auxiliary source preview:\n"
        f"         python scripts/thumbnail_html.py 'slide-*.html' \\\n"
        f"             {Path(pptx_path).stem}_preview --cols 4\n"
        "     HTML screenshots are useful for design review, but they are not a\n"
        "     substitute for PPTX render thumbnails or the text layout checker.\n\n"
        "  3. To get pixel-accurate thumbnails, install LibreOffice:\n"
...
def convert_to_images(pptx_path, temp_dir, dpi):
    ...
    try:
        ...
        raise RuntimeError(_soffice_missing_message(pptx_path)) from None
    if result.returncode != 0 or not pdf_path.exists():
        # `soffice` exists but failed (corrupt file, missing fonts, etc.).
        # Re-raise with the original stderr so the caller can debug.
        raise RuntimeError(
            "PDF conversion failed via soffice.\n"
            f"stderr: {(result.stderr or '').strip()}\n\n"
            + _soffice_missing_message(pptx_path, exists=True)
        )
    ...
    try:
        ...
        raise RuntimeError(
            "pdftoppm (poppler-utils) is required to rasterize the PDF.\n"
            "  macOS: brew install poppler\n"
            "  Linux: sudo apt-get install poppler-utils\n\n"
            + _soffice_missing_message(pptx_path)
        ) from None
    if result.returncode != 0:
        raise RuntimeError(
            f"pdftoppm failed: {(result.stderr or '').strip()}\n\n"
            + _soffice_missing_message(pptx_path, exists=True)
        )
