#!/usr/bin/env python3
"""
Create source-HTML preview grids by screenshotting HTML slides with Playwright.

This is an auxiliary, OPTIONAL preview path for the html2pptx workflow. It
screenshots the source HTML, not the final PPTX. Use it to review design and
composition, but do not treat it as proof that PowerPoint/WPS text layout is
safe. Always pair it with `inventory_textgrid.py --issues-only --fail-on-issues`
on the generated .pptx — that is the hard gate before delivery.

Pipeline:
    HTML files -> playwright-core connectOverCDP to the host's agent-browser
                  Chrome (the same bridge html2pptx.js uses; no bundled browser)
...
def _find_node_modules_root(start: Path) -> Path | None:
    ...
    `node_modules/playwright-core`. Returns that directory or None."""
    current = start.resolve()
    if current.is_file():
        current = current.parent
    for candidate in (current, *current.parents):
        if (candidate / "node_modules" / "playwright-core").is_dir():
            return candidate
    return None


def _resolve_cdp_url() -> str | None:
    """CDP websocket URL of the host's agent-browser Chrome.

    Prefer AGENT_BROWSER_CDP; otherwise ask the `agent-browser` CLI (opening a
    blank page first if there is no active session). Returns None if it cannot
    be resolved — the caller turns that into a human-readable error.
    """
    import os

    env = os.environ.get("AGENT_BROWSER_CDP", "").strip()
    if env:
        return env
    ...
    try:
        ...
        pass
    try:
        subprocess.run(["agent-browser", "open", "about:blank"], check=False,
                       capture_output=True)
        return _get() or None
    except Exception:
        return None
...
def render_html_files_via_node(
    ...
    """Fallback: use the Node playwright-core that html2pptx already depends on,
    rendering via connectOverCDP against the host's agent-browser Chrome — no
    bundled/launched browser.
    ...
    # Resolve a cwd that can `require('playwright-core')`. Prefer a directory
    # near the input HTML (typically the workspace where html2pptx ran its
    # bootstrap), then fall back to the script's own directory and finally
    ...
    if node_cwd is None:
        return None, (
            "could not locate `node_modules/playwright-core` near the HTML files, "
            "the script directory, or the current working directory. "
            'Run `bash "{skillDir}/bootstrap.sh"` in (or above) the workspace.'
        )

    cdp_url = _resolve_cdp_url()
    if not cdp_url:
        return None, (
            "could not obtain a CDP url from agent-browser. Ensure `agent-browser` "
            "is on PATH (the host provides it) or set AGENT_BROWSER_CDP."
        )
    ...
    (async () => {{
      const files = fs.readFileSync({str(list_file)!r}, 'utf-8')
        .split('\\n').filter(Boolean);
      const browser = await chromium.connectOverCDP({cdp_url!r});
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create source-HTML preview grids by screenshotting HTML slides with Playwright."
    )
    parser.add_argument(
        "inputs",
        nargs="+",
        ...
        if screenshots is None:
            ...
            if screenshots is None:
                print(
                    ...
                    f"      (Not used on the host — it ships no Python playwright.)\n"
                    f"  [2] Node playwright-core + agent-browser fallback: {node_error}\n"
                    f'      Fix: run `bash "{{skillDir}}/bootstrap.sh"` so '
                    f"`node_modules/playwright-core` exists, and ensure `agent-browser` "
                    f"is on PATH (or set AGENT_BROWSER_CDP).",
                    file=sys.stderr,
                )
                sys.exit(1)
