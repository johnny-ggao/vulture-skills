...
This skill ships with a pinned `package.json` for required Node deps (`pptxgenjs`, `playwright-core`, `sharp`, `react`, `react-dom`, `react-icons`).
...
- Idempotent: first run installs node deps (~30s); reruns finish in <100ms.
- **No browser download.** Rendering uses `playwright-core`'s `connectOverCDP` against the host's `agent-browser` Chrome — there is no bundled Chromium and no system-Chrome dependency. `agent-browser` is expected on `PATH` (the host provides it); `html2pptx.js` resolves the CDP url automatically (or honours `AGENT_BROWSER_CDP` if set).
...
### Text extraction
For plain text content, use the pure-Python text grid (uses `python-pptx`, which is
available on the host — no `markitdown`, no system tools):

```bash
python scripts/inventory_textgrid.py path-to-file.pptx -o content.md
```

Read the output in full (no range limits). For exact per-shape text + properties,
use `scripts/inventory.py` (see the template workflow) or unpack the OOXML and read
`ppt/slides/slide{N}.xml` (next section).
...
2. Create an HTML file for each slide with proper dimensions (e.g., 720pt × 405pt for 16:9)
   ...
   - Put visual box styling (`background`, `border`, `box-shadow`) on a wrapper `<div>` around text blocks; keep `<p>`, `<h*>`, `<ul>`, and `<ol>` for text and inline formatting.
...
4. **Validation**: Run the hard gate, then optionally a pixel preview.
   - **HARD GATE — PPTX text layout validation** (pure Python, always available):
     ```bash
     python scripts/inventory_textgrid.py output.pptx --issues-only --fail-on-issues
     ```
     This checks the generated `.pptx`, not just the source HTML. Any reported overflow/overlap must be fixed before delivery. The host has **no LibreOffice**, so this — not a rendered `.pptx` thumbnail — is the gate.
   - **Optional pixel preview — screenshot the source HTML** (design review only):
     ```bash
     python scripts/thumbnail_html.py 'workspace/slide-*.html' workspace/thumbnails --cols 4
     ```
     Renders via `playwright-core` + the host's `agent-browser` Chrome (same bridge as `html2pptx.js`). If `node_modules` is missing, run `bash "{skillDir}/bootstrap.sh"` and retry. For a single slide you can also use `agent-browser screenshot <path>` / `agent-browser pdf <path>` directly.
   ...
   - Inspect for: text cutoff (headers, shapes, edges), overlap, boundary spacing, contrast. Adjust HTML and regenerate until both the textgrid gate and the source preview are clean.
...
### Workflow
1. **Extract template text + layout**:
   ```bash
   python scripts/inventory_textgrid.py template.pptx -o template-textgrid.md
   ```
   * Pure Python (`python-pptx`); read the full file (no range limits). Gives per-slide shape bbox + text — enough to identify layouts, placeholder counts, and hierarchy.
   * The host has **no LibreOffice**, so a pixel-accurate thumbnail of an existing `.pptx` is not available; do template analysis from the text grid. (Pixel preview exists only for *source HTML* in the from-scratch workflow, via `thumbnail_html.py`.)
...
|------|-------|-------|--------|----------|
| `scripts/inventory_textgrid.py` | `.pptx` | Pure Python (`python-pptx`) | Markdown text grid | **Default on the host.** Hard gate for generated-PPTX layout issues; also the way to analyze an existing `.pptx` (no pixels). |
| `scripts/thumbnail_html.py` | HTML files (`slide-*.html`) | `playwright-core` + `agent-browser` | JPG grid of source HTML | html2pptx workflow — optional source preview, not final PPTX validation. |
| `scripts/thumbnail.py` | `.pptx` | LibreOffice (`soffice`) + Poppler (`pdftoppm`) | JPG grid (pixel-accurate) | Only off-host, where `soffice` is installed. **Not available on the host** (no LibreOffice). |

### `thumbnail_html.py` (source HTML preview)
...
- Renders via `playwright-core` + the host's `agent-browser` Chrome over CDP (the same bridge `html2pptx.js` uses; no bundled browser). The Python-`playwright` path is tried first and harmlessly skipped on the host.
- This screenshots source HTML, not the final `.pptx`; it cannot by itself satisfy final delivery validation.

### `thumbnail.py` (pixel JPG from .pptx — off-host only)
...
- Requires LibreOffice (`soffice`) + Poppler, which the host does **not** provide. On the host, use `inventory_textgrid.py` as the gate and `thumbnail_html.py` for an optional source preview instead.
...
Rendering an existing `.pptx` to pixels needs LibreOffice (`soffice`) + Poppler, which the host does **not** provide — so there is no pixel render of an existing `.pptx` on the host. Use:

- **Generated-PPTX QA** → `scripts/inventory_textgrid.py output.pptx --issues-only --fail-on-issues` (the hard gate).
- **Source-HTML preview** (from-scratch workflow) → `scripts/thumbnail_html.py 'workspace/slide-*.html' ...`, or `agent-browser screenshot <path>` / `agent-browser pdf <path>` for a single slide.

> Off-host, where `soffice` + `pdftoppm` exist, `thumbnail.py` can render a real `.pptx` to a JPG grid (and the two-step `soffice --headless --convert-to pdf` → `pdftoppm -jpeg -r 150` works manually). That path is not used on the host.
...
- `playwright-core` — drives the host's `agent-browser` Chrome over CDP (never downloads a browser) for html2pptx & thumbnail_html.py
- `sharp` — SVG rasterization / image processing
- `react` / `react-dom` / `react-icons` — JSX → HTML + icons

**Python** (provided by the host runtime — no `pip install` needed):
- `python-pptx` — used by inventory / textgrid / template scripts
- `defusedxml` + `lxml` — secure XML parsing for the OOXML edit path

**Browser**: the host's `agent-browser` (on `PATH`) supplies the Chrome that `playwright-core` connects to. No LibreOffice, no Poppler, no `markitdown`, no bundled/downloaded Chromium.
