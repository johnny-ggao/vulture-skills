...
**Text must go in text tags; visual box styles go on wrapper `<div>`s.**
- ✅ `<div style="background:#f0f0f0; border-left:4pt solid #333;"><p>Text here</p></div>`
- ❌ `<p style="background:#f0f0f0; border-left:4pt solid #333;">Text here</p>` / `<div>Text here</div>`
...
Deps (installed by `bootstrap.sh` into the skill's `node_modules`, resolved via `NODE_PATH`): `pptxgenjs`, `playwright-core`, `sharp`. Rendering connects to the host's `agent-browser` Chrome over CDP — no browser is launched or downloaded.
