...
6. **Recalc formulas** (MANDATORY if any formulas):
   ```bash
   # On the host (no LibreOffice): pure-Python path — preserves formulas, sets
   # fullCalcOnLoad, and scans for #DIV/0!/#VALUE!/#REF! via the `formulas` lib.
   python scripts/recalc_fallback.py output.xlsx
   ```
   If returned `status` is `errors_found`, fix cells in `error_summary` and re-run.
   (Off-host, where LibreOffice is installed, `python scripts/recalc.py output.xlsx` writes real cached values — see *Recalculating formulas*.)
...
### On the host — `scripts/recalc_fallback.py` (no LibreOffice)
The host has **no LibreOffice**, so this pure-Python path is the one to use:
```bash
...
- Statically scans formulas via the `formulas` library; reports the same error summary shape as `recalc.py`.
...
- `formulas` is declared in `requirements.txt` and installed by the host's auto-ensure — no manual `pip install`. If it is somehow absent, the scan is skipped (a `warning` is returned) but `fullCalcOnLoad` still runs.

### Off-host only — `scripts/recalc.py` (drives LibreOffice)
Where LibreOffice (`soffice`) is installed, this writes **real cached values** back (the host cannot):
```bash
...
- On the host it just reports `LibreOffice (soffice) is not installed` and points back to the fallback above.
